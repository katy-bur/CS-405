...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    for (unsigned long int i = 0; i < steps; ++i)
    {
        ...
        { 
            return result + std::numeric_limits<T>::min(); 
        }
        result += increment;
    }
...
void test_overflow()
{
    ...
    T result2 = add_numbers<T>(start, increment, steps + 1);
    if (result2 - std::numeric_limits<T>::min() == result)
    {
        std::cout << +(result2 - std::numeric_limits<T>::min()) << std::endl; std::cout << "\tOverflow Ocurred" << std::endl;
    }
    else 
    { 
        std::cout << +(result2) << std::endl; std::cout << "\tNo Overflow Ocurred" << std::endl; 
    }
    //std::cout << +result << std::endl;
}
...
void test_underflow()
{
    ...
    T result2 = subtract_numbers<T>(start, decrement, steps + 1);
    if (result2 == std::numeric_limits<T>::max()) {
        std::cout << +(result) << std::endl;
        std::cout << "\tUnderflow Ocurred" << std::endl;
    }
    else 
    { 
        std::cout << +(result2) << std::endl;
        std::cout << "\tNo Underflow Ocurred" << std::endl; 
    }
    //std::cout << +result << std::endl;
}
